        ...
        coreDataManager.mainManagedObjectContext.performAndWait {
            ...
            do {
                ...
                for bgReading in bgReadings {
                    ...
                    returnValue.append(BgReadingSnapshot(timeStamp: bgReading.timeStamp, calculatedValue: bgReading.calculatedValue, rawData: bgReading.rawData, finalValue: bgReading.finalValue, adjustedValue: bgReading.adjustedValue?.doubleValue, smoothedValue: bgReading.smoothedValue?.doubleValue, backfilledAt: bgReading.backfilledAt, calculatedValueSlope: bgReading.calculatedValueSlope, hideSlope: bgReading.hideSlope, sensorTrendOrdinal: bgReading.sensorTrendOrdinal?.intValue, id: bgReading.id, deviceName: bgReading.deviceName, calibrationSnapshot: calibrationSnapshot, sensorID: bgReading.sensor?.id, objectID: bgReading.objectID))
